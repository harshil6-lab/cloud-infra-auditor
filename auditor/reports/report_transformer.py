RESOURCE_CONFIG = {
    ...
    "EIP": {"id_field": "AllocationId", "status": "Unassociated"},
}


def build_ec2_details(item):
    return {"instance_type": item["InstanceType"], "state": item["State"]}


def build_ebs_details(item):
    return {"size_gib": item["Size"], "state": item["State"]}


def build_eip_details(item):
    return {"public_ip": item["PublicIp"]}
...
def transform_report(report):
    ec2 = transform_findings(report["ec2"]["findings"], "EC2", build_ec2_details)
